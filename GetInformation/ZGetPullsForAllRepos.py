...
class GetPullsForAllRepos:
    ...
    def initlog(self):
        ...
        self.logfile=os.path.join(GetFilePathRoot.get_root_dir(), "data2", "GetAllPulls.txt")
        self.log(None)
        self.outputfile = os.path.join(GetFilePathRoot.get_root_dir(), "data2", "GetAllPullsOutput.txt")
        import logging
        # 创建一个logger
    ...
    def doalldeal(self):
        ...
        from DealMongo import DealMongo
        i=0
        import time
        for d in self.do():
            start = time.time()
